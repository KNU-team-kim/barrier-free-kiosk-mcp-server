...
from steps.resident_registration_steps import RESIDENT_REGISTRATION_STEPS
...
async def retry_step_by_index(index, STEPS, agent_chain, chat_history, output, ctx, session_id):
    step_name, _, step_prompt = STEPS[index]
    ...
    while step_output == "nothing":
        try:
            ...
            if ai_output.data:
                if step_name == "phone_number" and not fetch_check_phone_number(ai_output.data, output.name):
                    ...
                    ai_output.data = fetch_retrieve_address(output.name, output.phone_number)
                ...
                elif step_name == "registration_number" and not fetch_check_registration_number(ai_output.data):
    ...
    if hasattr(output, step_name):
        setattr(output, step_name, step_output)
...
async def move_in_conversation(ctx: Context, session_id: Optional[str] = None) -> Output:
    ...
    for idx, (step_name, _, step_prompt) in enumerate(MOVE_IN_STEPS):
        user_message = ""
        step_output = "nothing"
        retrieve_output = True
        ...
        retry_steps = ""
        for i in range(idx):
            retry_steps += f"{i}: {MOVE_IN_STEPS[i][1]}\n"
        ...
        while step_output == "nothing":
            try:
                ...
                if ai_output.retry != None:
                    await retry_step_by_index(
                        index=ai_output.retry, 
                        STEPS=MOVE_IN_STEPS,
                        agent_chain=agent_chain, 
                        chat_history=chat_history, 
                        output=move_in_output, 
                        ctx=ctx, 
                        session_id=session_id
                    )
                    continue
...
async def resident_registration_conversation(ctx: Context, session_id: Optional[str] = None) -> Output:
    ...
    for idx, (step_name, _, step_prompt) in enumerate(RESIDENT_REGISTRATION_STEPS):
        user_message = ""
        step_output = "nothing"
        retrieve_output = True
        ...
        retry_steps = ""
        for i in range(idx):
            retry_steps += f"{i}: {RESIDENT_REGISTRATION_STEPS[i][1]}\n"
        ...
        while step_output == "nothing":
            try:
                ...
                if ai_output.retry != None:
                    await retry_step_by_index(
                        index=ai_output.retry, 
                        STEPS=RESIDENT_REGISTRATION_STEPS,
                        agent_chain=agent_chain, 
                        chat_history=chat_history, 
                        output=resident_registration_output, 
                        ctx=ctx, 
                        session_id=session_id
                    )
                    user_message = ""
